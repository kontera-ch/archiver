...
export class ArchivalController {
  ...
  async archiveBucketFile(@Body() fileArchivalDTO: FileArchivalDTO) {
    ...
    // ToDo: check if file is already archived
    ...
    this.archivalService.stamp(sha256Hash)

    const archivalState = await this.googleCloudStorageService.archiver.archiveFile(file, { sha256Hash })
